...
const weatherDisplay = document.querySelector('.grid-container')
const cityList = document.querySelector('.cityList')
let cities = []

// the date
function displayDate (day) {
  const todayDate = dayjs().format('M/DD/YYYY')
  return todayDate
}
...
function newButton () {
  ...
  button.onclick = function (event) {
    ...
    fetch(secondFetch)
      ...
      .then((data) => {
        const requestTwo = `https://api.openweathermap.org/data/2.5/forecast?q=${searchBarText}&appid=${apiKey}&units=imperial`
        fetch(requestTwo)
          .then((response) => response.json())
          .then((data) => {
...
function fetchData (searchBarText) {
  ...
  const results = fetch(dataFetch)
    .then((response) => response.json())
    .then((data) => {
      const requestUrl = `https://api.openweathermap.org/data/2.5/forecast?q=${searchBarText}&appid=${apiKey}&units=imperial`

      const fetchAgain = fetch(requestUrl)
        .then((response) => response.json())
        .then((data) => {
          currentConditions(data)
          fiveDay(data)
...
function currentConditions (citydata) {
  const day = dayjs().format('M/DD/YYYY')
  const todaysDate = 'Right now'
  const todayDiv = document.createElement('div')
  const currentHeader = document.createElement('h2')
  const listConditions = document.createElement('ul')
  ...
  const iconURL =
    ...
    citydata.list[0].weather[0].icon +
    '.png'
...
function fiveDay (citydata) {
  ...
  for (let i = 1; i < futureCast.length; i += 8) {
    ...
    const day = dayjs(futureCast[i].dt_txt).format('M/DD/YYYY')
    const weatherIcon = futureCast[i].weather[0].icon
